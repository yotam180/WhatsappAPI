	...
	window.API = {
		...
		*/
		sendTextMessage: function(chat_id, message_text, callback) {
			var chat = Core.chat(chat_id);
			if (chat == null) {
				let contact = Core.contact(chat_id);
				if (contact==null){
					Core.error(API.Error.CHAT_NOT_FOUND, callback)
					return;
				}
				...
				{
					Store.Chat._staleCollection=true;
					Store.Chat._serverQuery.call(Store.Chat,'FIND',contact.__x_id).then(()=>{
						chat = Core.chat(chat_id);
						Store.SendTextMsgToChat(chat,message_text);
						Store.Chat._staleCollection=false;
						return;
					});
					return;
				}
